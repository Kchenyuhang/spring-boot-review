package com.soft1851.springboot.quartz.mananger;
...
public class QuartzManager {
    ...
    public static void addJob(String jobName, String jobGroupName, String triggerName,
                              ...
                              String cron, Object... objects) throws SchedulerException {
...
//        int oldInterval = (int) trigger.getRepeatInterval();
        String oldCron = String.valueOf(trigger.getScheduleBuilder());
        //判断老的间隔和新的间隔是不是一样
        if (cron != oldCron) {
            Date startTime = new Date();
//            startTime.setTime(startTime.getTime() + interval * 1000);
            Trigger newTrigger = TriggerBuilder.newTrigger().withIdentity(triggerName, triggerGroupName)
                    .startAt(startTime).withSchedule(CronScheduleBuilder.cronSchedule(cron))
                    .build();
            // 修改任务的触发时间
            scheduler.rescheduleJob(triggerKey, newTrigger);
            log.info("触发器{}修改,新的执行间隔为{}", triggerName, cron);
        }
    }
